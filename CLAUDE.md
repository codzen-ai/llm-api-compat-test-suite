...
4. **On teardown**, client fixture writes per-test `.log` files into `reports/{timestamp}/{model}/logs/`; `pytest_sessionfinish` writes one `summary.md` per model under its subdirectory plus a top-level `reports/{timestamp}/index.md` listing all models — see [docs/design/multi-model-report-layout.md](/docs/design/multi-model-report-layout.md) for why the per-model split exists and why the index deliberately omits a pass-rate column
...
- `--profile <name>` is required (YAML per-model or CLI flag); capabilities are read from `model_profiles/{api_format}/{profile}/…yaml`, not user-declared — see [docs/design/profile-based-compatibility-testing.md](/docs/design/profile-based-compatibility-testing.md)
- **Reports are split per-model**: `reports/{ts}/{model}/{summary.md + logs/}` plus a top-level `index.md`. The index lists models but does not compute pass rates because different models typically have different capability sets — see [docs/design/multi-model-report-layout.md](/docs/design/multi-model-report-layout.md)
...
Put it in the right `tests/{format}_compat/` directory. Attach `@pytest.mark.capability("X")` for **every** parameter/behavior the test exercises — marker granularity is "one capability per independently-toggleable model behavior" (see [docs/design/profile-based-compatibility-testing.md](/docs/design/profile-based-compatibility-testing.md)). Register new marker names in `pyproject.toml`'s `markers` list. Request `client` and `model` fixtures:
